...
// export connect  (data: 2023 / 5 / 30)
void ExportConnect(string path_, DoubleArray& stress)
{
	string path = path_ + "stress.txt";
	std::ofstream fout(path);
	if (!fout)
	{
		std::cerr << "fail to open file" << std::endl;
	}
	int ne = stress.getSize();
	int id = 1;
	for (int i = 1; i <= ne; i++)
	{
		fout << stress(i)<< std::endl;
	}
}

// write VYK file (data: 2023 / 5 / 29)
void ExportData2VtkFile(string path_, double factor, DoubleMatrix& coord, IntMatrix& connect, 
	DoubleArray& dis, IntArray& node_map, DoubleArray& stress)
{
	...
	// write infom of von Mises stress
