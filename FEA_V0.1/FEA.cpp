...
void FEA::Process(string  read_path, string  save_path, double em, double nu)
{
	...
	*/
	DoubleArray stress(n_elem_);
	PostProcessStress post;
	post.GetStress(node_map, connect_, coord_, dis, stress, mat_);
	...
	ExportConnect(save_path, stress);
	ExportData2VtkFile(save_path, 5.0e1, coord_, connect_, dis, node_map, stress);
}
