...
public:
	...
	/**
	 * \brief K(this) * A(answer) = B, 求解A矩阵
	 * \param B RHS矩阵
	 * \param answer A
	 * \param transpose 转置标记
	 * \return 返回this（K矩阵）的奇异性，false 是K矩阵是奇异的
	 */
	bool solveForRhs(const DoubleMatrix& B, DoubleMatrix& answer, bool transpose = false);
	/**
	 ...
	 * \param scale 放大倍数
	 */
	void plusDyadicUnsymm(const DoubleArray& a, const DoubleArray& b, double scale = 1.);
